...
if (!path) {
  console.error('Usage: node format-spec-md.mjs <path-to.md>')
  process.exit(1)
}
...
/** 支持 - 1.1.【推荐】、1.5.【推荐】（无列表符）、- 【强制】（无编号）等写法 */
const ruleReNumbered = /^-? ?(\d+(?:\.\d+)*)\.?【(强制|推荐|参考)】(.*)$/
const ruleRePlain = /^- 【(强制|推荐|参考)】(.*)$/
const h2Re = /^## (\d+)\.\s*(.+)$/
const h3Re = /^### (\d+\.\d+)\.\s*(.+)$/
...
function parseRuleLine(line) {
  const numbered = line.match(ruleReNumbered)
  if (numbered) return { num: numbered[1], tag: numbered[2], rest: numbered[3] }
  const plain = line.match(ruleRePlain)
  if (plain) return { num: null, tag: plain[1], rest: plain[2] }
  return null
}

function isRuleLine(line) {
  return parseRuleLine(line) !== null
}
...
function flushRule(out, tag, num, rest, bodyLines) {
  const title = num
    ? `${normalizeNum(num)}${rest.startsWith(' ') ? '' : ' '}${rest}`.trimEnd()
    : rest.trimStart()
  if (tag === '强制' || tag === '推荐') {
    out.push(`::: ${tag}`)
    out.push(title)
    ...
    out.push(`${numOut}【参考】${rest}`.trimEnd())
  }
  out.push('')
...
function preprocessTipBlock(src) {
  const out = []
  let i = 0
  while (i < src.length) {
    if (src[i].trim() === ':::tip' || src[i].trim() === '::: tip') {
      const body = []
      i++
      while (i < src.length && src[i].trim() !== ':::') {
        body.push(src[i].trim())
        i++
      }
      if (i < src.length) i++
...
lines = preprocessTipBlock(lines)
...
while (i < lines.length) {
  ...
  if (isRuleLine(line)) {
    const { num, tag, rest } = parseRuleLine(line)
    const bodyLines = []
    i++
    while (i < lines.length) {
      const next = lines[i]
