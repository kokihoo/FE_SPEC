...
::: 推荐
1.1. 使用 `Node.js` 内置的全局变量。`eslint`: [node/prefer-global](https://github.com/mysticatea/eslint-plugin-node/blob/v11.1.0/docs/rules/prefer-global)
:::
...
::: 推荐
1.2. 使用模块内支持的 `promises` API。`eslint`: [node/prefer-promises](https://github.com/mysticatea/eslint-plugin-node/tree/master/docs/rules/prefer-promises)
:::
...
::: 推荐
1.3. 如无特殊需求，模块引用声明放在文件顶端，注意引用顺序。`eslint`: [import/order](https://github.com/benmosher/eslint-plugin-import/blob/master/docs/rules/order.md)
:::
...
::: 推荐
1.4. 抛出异常时，使用原生 `Error` 对象。`eslint`: [no-throw-literal](https://eslint.org/docs/rules/no-throw-literal)
:::
...
::: 推荐
1.5. 线上环境尽量不要使用 `fs/child_process` 模块的 `sync` 方法，如 `fs.readFileSync()`、`cp.execSync()` 等。
:::
...
::: 强制
2.4. 禁止使用从参数或明文 cookie 中获取的用户标识进行敏感信息查询输出。
:::
...
::: 推荐
2.7. 用户上传文件不允许至服务器本地，需要上传到 OSS 等服务。
:::
...
::: 推荐
3.5. 使用 `util.promisify` 处理回调函数，使其返回 `Promise`。
:::
...
::: 推荐
3.6. 使用 Node.js 原生 `Promise`，而不是三方库如 `bluebird`。
:::


::: 推荐
3.7. 在类方法中返回 `this` 方便链式调用。
:::
